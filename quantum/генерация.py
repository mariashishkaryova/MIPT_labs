...
for s in range(0, 2):
    G = 3 * 10 ** 11
    A0 = np.random.uniform(0.9, 2.5) #амплитуда смещений, m
    vr = np.random.uniform(1000, 2000) #скорость распространения, m/s

    a = np.random.uniform(10000, 20000)  # m
    k = np.random.uniform(0.2, 5)  # = b / a, m
    b = k * a
    ...
    xx0 = g * x0 #точка максимума гауссового распределения, м
    yy0 = 0
